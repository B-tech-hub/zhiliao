...
**`Esc` 的分层退出靠监听位置实现，不靠优先级判断。** 助手面板的监听挂在 `window` 上，任何后来的浮层若也挂 `window`，一次 `Esc` 就会关掉两层。新增浮层一律把监听挂在自己的 DOM 节点上并 `stopPropagation`，事件走不到 `window`，分层才成立。命令面板是例外——它挂 `window` 捕获阶段（要先于一切拿到 `⌘K`），但关闭状态下对 `Esc` 直接 return，不参与竞争。
...
**草稿必须托管在浮层而不是表单里**：表单随浮层开合挂载卸载，正文存在组件内部时误触关闭一次就没了。表单的 content 因此是可选受控（两个 prop 都不传时行为与整页一致），保存成功才清空。点遮罩一律不关闭——里面躺着没保存的字；遮罩的 `onMouseDown` 还要 `preventDefault`，否则焦点飘到 `body` 上，`Esc` 就走不到挂在浮层节点上的监听器。
