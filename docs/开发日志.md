...
- **质量基线**：Vitest 253 例全绿（v0.2.1 时为 71 例，本阶段新增 182 例）、`tsc` ✅、lint ✅、`npm run build` ✅。
- **验证方式**：单元测试 + 变异检验为主；demo 隔离环境（`./data-demo/`，3001 端口）走完全部动线——建笔记→撤销、删除→拒绝/允许→撤销、刷新重开重建卡片与置灰态、附件摘除与加回（查库确认 scope 切换）、引用真假 id、降级提示两态。真实数据 `./data/` 全程未触碰。
- **交付边界**：v0.3 的功能主干已完成，**图像生成、Mermaid、每周回顾按计划推迟到 v0.4**。
- **发版前还差**：`package.json` 仍是 `0.2.0`（v0.2.1 的改动攒着跟 v0.3 一起发）；本地提交尚未推送；README 界面预览的 4 张截图右下角缺助手悬浮钮，内容不算错但不完整，已决定本批次不动；**未做过真实模型的提示注入实测**（mock LLM 不会被说服），接入真实供应商后应补一次。
