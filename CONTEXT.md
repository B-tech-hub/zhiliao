...
### 主题（Topic）
笔记的唯一归类维度，扁平一层。创建入口在首页标题栏、侧栏主题列表底部与 设置 → 主题管理，三处共用同一接口。其中**未分类**是系统内置主题，不可删除、不可重命名。删除普通主题时，其下笔记回落到未分类。
...
### 导入（Import）
从 zip 批量写入笔记。自家导出包按 id 恢复主题、标签、时间、摘要与图片；普通 Markdown 的标题按 front-matter `title`、首个 H1、文件名依次推断，主题按 `topic`、`category`、直接父目录依次推断。没有 id 时用文件内容指纹判重：同一内容重复导入会跳过，内容变化则保守地视为新笔记。默认不跑 AI 整理。
...
### 手写摄取
单张手写图片可追加到已有笔记并进入 `handwriting_transcribe` 队列；原图保留在正文中，转写结果以“待核对”块追加，完成后再进入 `note_process`。独立新笔记入口复用同一任务。视觉调用期间若正文发生变化，结果保存为候选稿，用户明确接受后才追加。

### 转写审核状态与公式块
笔记独立记录 `unreviewed/reviewed/needs_review` 转写审核状态及告警。数学公式使用 `$...$`/`$$...$$` Markdown 源码，阅读态 KaTeX 渲染，源码始终可导出和往返编辑。

### 主题建议
未分类笔记积攒后，AI 聚类产出的"新建/归入主题"建议，用户可逐条采纳或全部忽略。采纳是**逐项**的：只消费当前这一项，其余建议继续待处理，其中已被迁走的笔记自动剔除，剩不到 2 条笔记的建议随之作废（同一条笔记可能同时出现在多个候选主题里）。
